...
def detect_triangles(frame, color_name, min_area=700):
    ...
    for cnt in contours:
        ...
        if area > min_area:
            ...
            cv2.drawContours(frame, [cnt], 0, (255, 0, 0), 2)  # Draw all contours for debugging
            if len(approx) == 3:
                detected_triangles.append(approx)
                cv2.drawContours(frame, [approx], 0, (0, 255, 0), 2)  # Draw detected triangle

            # Debugging information
            print(f"Contour Area: {area}, Sides: {len(approx)}")
